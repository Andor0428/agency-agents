# Wispr Flow transcription (WhisperFlow-style voice)

Stock Take can use **[Wispr Flow](https://wisprflow.ai)** for speech-to-text — the same family of tech people use in the WhisperFlow / Wispr keyboard apps. It tends to handle spirit names and casual speech better than raw Groq Whisper.
...
Phone (Hold to Talk)
  → stock-take-api  POST /api/device/voice/transcribe
      → ffmpeg (m4a → 16 kHz wav)
      → Wispr Flow REST API (dictionary = your catalog spirits)
...
3. See [api-docs.wisprflow.ai](https://api-docs.wisprflow.ai)
