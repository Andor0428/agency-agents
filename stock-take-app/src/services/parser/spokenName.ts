...
const FILLER_WORDS =
  /\b(?:please|thanks|thank you|cheers|mate|bottle|bottles|btl|case|cases|unit|units|each|of|the|a|an|some|about|roughly|maybe|around|like|got|have|has|is|are|at|in|on|shelf|bar|back bar|speed rail|well|top shelf|there|their|theyre|they're|here|where)\b/gi;

const NUMBER_WORDS = /\b(?:one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|a|an|\d+(?:\.\d+)?)\b/gi;

/** Strip accents and smart quotes from Whisper output. */
...
export function normalizeSpokenProductName(spoken: string): string {
  let text = normalizeTranscriptText(spoken.trim());
  if (!text) return text;
